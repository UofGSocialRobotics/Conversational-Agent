---
title: "healthRecSys data analysis objective eval"
...
data_no_tags <- subset(data, tags_mode == "no-tags")
data_tags <- subset(data, tags_mode == "tags")
```
...
```{r}
as.table(with(data_reco_health_scores,by(reco.health.score,file,function(xx)names(which.max(table(xx))))))
```
...
ANOVA for F1
```{r}
model_no_tags = lm(f1 ~ reco_mode, data = data_no_tags)
...
ANOVA for F1s
```{r}
model_tags = lm(f1 ~ reco_mode, data = data_tags)
